...
# Introduction
This is my portfolio as a front-end developer that is built with React to demonstrate my skills. It is a single-page application which showcases my work and contain the following sections:
...
To run and host the app, make sure Node.js, NPM are installed in your local device and simply run `npm build` to install all the necessary packages for serving the app. Ensure the terminal is pointing to the root directory of the project folder as its target directory. `npm start` when all the packages are installed properly.


## Libraries
The react application is built with some popular external open source libraries that provides extensive help to me to create a more polished and professional looking page in a more effective approach.
